...
import { MAJOR_TONICS_ORDER, MINOR_TONICS_ORDER, getTonicDisplayLabel } from '../music/keys';
...
const CircleOfFifths: React.FC<CircleOfFifthsProps> = ({ onSelect, selectedTonic, selectedMode }) => {
  ...
  const sectorInnerDefault = theme.palette.mode === 'dark' ? theme.palette.grey[800] : theme.palette.grey[100];
  const hoverFill = alpha(theme.palette.primary.light, theme.palette.mode === 'dark' ? 0.28 : 0.18);
  const sectorStroke = theme.palette.divider;
  const centerFill = theme.palette.background.default;

  return (
    <Box display="flex" justifyContent="center" alignItems="center" sx={{ width: '100%', aspectRatio: '1/1', mx: 'auto', my: 2 }}>
      <svg viewBox={`0 0 ${SVG_SIZE} ${SVG_SIZE}`} width="100%" height="100%" preserveAspectRatio="xMidYMid meet" style={{ display: 'block' }}>
        <g transform={`rotate(-15 ${CENTER} ${CENTER})`}>
          ...
          {MAJOR_TONICS_ORDER.map((tonic, i) => {
            ...
            const isSelected = selectedTonic === tonic && selectedMode === 'Ionian';
            return (
              <g
                key={tonic}
              ...
              >
                <path
                  d={describeArc(CENTER, CENTER, INNER_RADIUS, OUTER_RADIUS, start, end)}
                  fill={isSelected ? selectedFill : (hoverMajor === i ? hoverFill : sectorDefault)}
                  stroke={sectorStroke}
          ...
          {MINOR_TONICS_ORDER.map((tonic, i) => {
            const start = (i * 360) / SECTORS;
            const end = ((i + 1) * 360) / SECTORS;
            const isSelected = selectedTonic === tonic && selectedMode === 'Aeolian';
            return (
              <g
                key={tonic}
                style={{ cursor: 'pointer' }}
              ...
              >
                <path
                  ...
                  fill={isSelected ? selectedFill : (hoverMinor === i ? hoverFill : sectorInnerDefault)}
                  stroke={sectorStroke}
          ...
          <circle cx={CENTER} cy={CENTER} r={48} fill={centerFill} stroke={sectorStroke} strokeWidth={1.5} />
        </g>
        ...
        <g>
          {MAJOR_TONICS_ORDER.map((tonic, i) => {
            ...
            const pos = polarToCartesian(CENTER, CENTER, (OUTER_RADIUS + INNER_RADIUS) / 2, angleForLabel);
            const isSelected = selectedTonic === tonic && selectedMode === 'Ionian';
            return (
              <text
                key={tonic + '-label'}
          ...
          {MINOR_TONICS_ORDER.map((tonic, i) => {
            ...
            const mid = (start + end) / 2;
            const angleForLabel = mid - 15;
            const pos = polarToCartesian(CENTER, CENTER, (INNER_RADIUS + 40) / 2, angleForLabel);
            const isSelected = selectedTonic === tonic && selectedMode === 'Aeolian';
            return (
              <text
